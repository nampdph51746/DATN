                                        ...
                                        <select name="customer_rank_id" id="customer_rank_id"
                                            ...
                                            @foreach (\App\Models\CustomerRank::all() as $rank)
                                                <option value="{{ $rank->id }}"
                                                    {{ old('customer_rank_id') == $rank->id ? 'selected' : '' }}>
                                        ...
                                        <select name="promotion_id" id="promotion_id"
                                            ...
                                            @foreach (\App\Models\Promotion::all() as $promotion)
                                                <option value="{{ $promotion->id }}"
                                                    ...
                                                    {{ $promotion->name ?? ($promotion->code ?? $promotion->id) }}</option>
                                            @endforeach
                                        </select>
                                        @error('promotion_id')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        ...
                                        <div class="d-flex gap-2">
                                            <a href="{{ route('customer_rank_promotions.index') }}"
                                                ...
                                                <iconify-icon icon="solar:arrow-left-broken"
                                                    class="align-middle fs-18"></iconify-icon> Quay lại
                                            </a>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        <form method="POST" action="{{ route('customer_rank_promotions.store') }}">
                                            ...
                                            <div class="row">
                                                <div class="col-lg-6">
                                                    <div class="mb-3">
                                                        <label for="customer_rank_id" class="form-label">Hạng khách
                                                            hàng</label>
                                                        <select name="customer_rank_id" id="customer_rank_id"
                                                            class="form-control @error('customer_rank_id') is-invalid @enderror">
                                                            <option value="">Chọn hạng khách hàng</option>
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="promotion_id" id="promotion_id"
                                                            ...
                                                            @foreach (\App\Models\Promotion::all() as $promotion)
                                                                <option value="{{ $promotion->id }}"
                                                                    {{ old('promotion_id') == $promotion->id ? 'selected' : '' }}>
                                                                    {{ $promotion->name ?? ($promotion->code ?? $promotion->id) }}
                                                                </option>
                                                            @endforeach
                                                        </select>
                                                        @error('promotion_id')
