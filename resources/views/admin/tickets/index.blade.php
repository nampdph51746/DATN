                    ...
                    <div class="card-header d-flex justify-content-between align-items-center gap-1">
                        ...
                        <div class="d-flex gap-2 align-items-center">
                            ...
                            <form method="GET" action="{{ route('tickets.index') }}" class="d-flex align-items-center gap-2">
                                <input type="text" name="search" placeholder="Tìm kiếm ID vé hoặc ID suất chiếu"
                    ...
                    <div>
                                    ...
                                    @forelse($tickets as $ticket)
                                        <tr>
                                            <td>
                                                <div class="form-check">
                                                    <input type="checkbox" class="form-check-input"
                                                        id="ticketCheck{{ $ticket->id }}">
                                                    <label class="form-check-label"
                                                        for="ticketCheck{{ $ticket->id }}"></label>
                                                </div>
                                            </td>
                                            <td>{{ $ticket->id }}</td>
                                            ...
                                            <td>
                                                ...
                                                @switch($status)
                                                    @case('valid')
                                                        <span class="badge bg-success-subtle text-success px-2 py-1 fs-13">Hợp
                                                            lệ</span>
                                                    @break

                                                    @case('used')
                                                        <span class="badge bg-primary-subtle text-primary px-2 py-1 fs-13">Đã sử
                                                            dụng</span>
                                                    @break
                        ...
                        <td>
                            <div class="d-flex gap-2">
                                <!-- View Button -->
                                <a href="{{ route('tickets.show', $ticket->id) }}" class="btn btn-light btn-sm"
                                    title="Xem chi tiết">
