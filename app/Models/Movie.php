...
class Movie extends Model
{
            ...
            ->whereHas('tickets', function($query) {
                $query->whereNotNull('booking_id')
                      ->whereHas('booking', function($bookingQuery) {
                          // Chỉ tính những booking có trạng thái confirmed, completed hoặc checked_in
                          $bookingQuery->whereIn('status', ['confirmed', 'completed', 'checked_in']);
                      });
            })
            ->exists();
    }
    ...
    public function canBeEdited()
    {
        ...
        // Không thể chỉnh sửa nếu phim sắp chiếu nhưng đã có suất chiếu
        if ($movieStatus === 'upcoming' && $this->showtimes()->exists()) {
            return false;
        }
