                    ...
                    <div class="card-header">
                                    ...
                                    <form class="d-flex align-items-center gap-2" method="GET"
                                        ...
                                        <div class="position-relative">
                                            <input type="search" name="search"
                                                class="form-control form-control-sm ps-4 pe-3" style="min-width: 250px;"
                                                placeholder="Tìm kiếm tên phòng, rạp chiếu..." autocomplete="off"
                                        ...
                                        <select name="status" class="form-select form-select-sm" style="width:120px;">
                                            ...
                                            <option value="maintenance"
                                                {{ request('status') == 'maintenance' ? 'selected' : '' }}>Bảo trì</option>
                        ...
                        <form id="delete-selected-form" action="#" method="POST" style="display: none;" class="mt-2">
                            ...
                            <button type="submit" class="btn btn-danger btn-sm"
                                onclick="return confirm('Bạn có chắc muốn xóa các phòng đã chọn?')">
                            ...
                            <table class="table align-middle mb-0 table-hover table-centered">
                                <thead class="bg-light-subtle">
                                    <tr>
                                        ...
                                        <th style="width: 100px;">Suất Chiếu</th>
                                        <th style="width: 120px;">Trạng Thái</th>
                                        {{-- <th style="width: 80px;">ID</th> --}} {{-- Bỏ cột ID --}}
                                ...
                                <tbody>
                                    @foreach ($rooms as $room)
                                        <tr>
                                                ...
                                                <div class="form-check ms-1">
                                                    @if (!$room->canBeEdited())
                                                        <input type="checkbox" class="form-check-input" disabled
                                                               title="Không thể chọn - phòng đang có suất chiếu hoạt động">
                                                    @else
                                            ...
                                            <td>
                                                <span
                                                    class="badge bg-info-subtle text-info">{{ $room->roomType->name }}</span>
                                            </td>
                                            <td>
                                            ...
                                            <td>
                                                ...
                                                @if ($activeShowtimes > 0)
                                                    <span class="badge bg-warning text-dark">{{ $activeShowtimes }} suất</span>
                                                ...
                                                <div class="d-flex gap-2 justify-content-center">
                                                    ...
                                                    @if (!$room->canBeEdited())
                                                        <span class="btn btn-soft-secondary btn-sm disabled" 
                                                              title="Không thể chỉnh sửa - phòng đang có suất chiếu hoạt động">
                                                            <i class="bi bi-pencil"></i> Chỉnh sửa
                                                        </span>
                                                    @else
                                                        <a href="{{ route('admin.rooms.editStatus', $room->id) }}"
                                                    ...
                                                    @can('delete room')
                                                        @if (!$room->canBeEdited())
                                                            <span class="btn btn-soft-secondary btn-sm disabled" 
                                                                  title="Không thể xóa - phòng đang có suất chiếu hoạt động">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.getElementById('delete-selected-form').addEventListener('submit', function(e) {
                const checked = Array.from(document.querySelectorAll('.room-checkbox:checked')).map(cb => cb
                    .value);
                if (checked.length === 0) {
                    e.preventDefault();
                    return false;
