...
<div class="container-fluid px-4 py-4">
                    ...
                    <div class="d-flex align-items-center justify-content-between">
                        <div class="d-flex align-items-center gap-4">
                            ...
                            <div class="text-white">
                                <h2 class="fw-bold mb-2">Chỉnh sửa diễn viên</h2>
                                <p class="mb-0 opacity-90 fs-5">Cập nhật thông tin diễn viên: {{ $actor->name }}</p>
                            </div>
                        </div>
    ...
    <div class="row g-4">
            ...
            <div class="card border-0 shadow-lg rounded-4 sticky-top">
                ...
                <div class="card-body p-4 text-center">
                    <div class="position-relative d-inline-block mb-4">
                        <img id="imagePreview" 
                             src="{{ $actor->image_path ? Storage::url($actor->image_path) : asset('assets/images/actor-placeholder.png') }}" 
                             alt="Actor Preview" 
                    ...
                    <div class="upload-area border-2 border-dashed border-warning rounded-4 p-4 bg-warning-subtle cursor-pointer" 
                        ...
                        <div class="upload-content text-center">
                            <i class="bi bi-cloud-upload text-warning fs-1 mb-3"></i>
                            <h6 class="fw-bold text-warning mb-2">Thay đổi ảnh diễn viên</h6>
                        ...
                        <div class="mt-3">
                            <div class="form-check">
                                ...
                                <label for="remove_image" class="form-check-label text-danger">
                                    <i class="bi bi-trash3 me-1"></i>Xóa ảnh hiện tại
                                </label>
                            </div>
                        </div>
                    @endif
                ...
                <div class="card-footer bg-white border-0 py-4">
                    <div class="d-grid gap-2">
                        <button type="submit" form="actorEditForm" class="btn btn-warning btn-lg rounded-pill">
                            <i class="bi bi-save me-2"></i>Cập nhật diễn viên
                        </button>
                        <a href="{{ route('admin.actors.show', $actor->id) }}" class="btn btn-outline-info btn-lg rounded-pill">
        ...
        <div class="col-xl-8 col-lg-7">
            <form id="actorEditForm" action="{{ route('admin.actors.update', $actor->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                                ...
                                <div class="form-floating">
                                    <div class="form-control d-flex align-items-center" style="height: calc(3.5rem + 2px);">
                                        <span class="text-muted" id="imageFileName">
                                            {{ $actor->image_path ? 'Ảnh hiện tại: ' . basename($actor->image_path) : 'Chưa chọn file ảnh' }}
                                        </span>
                                    </div>
                                    <label>
                    ...
                    <div class="card-body p-4">
                        <div class="form-floating">
                            <textarea name="biography" 
                                      ...
                                      style="height: 150px;">{{ old('biography', $actor->biography) }}</textarea>
                            <label for="biography">
                    ...
                    <div class="card-body p-4">
                        <div class="form-check form-switch">
                            <input type="checkbox" 
                                   ...
                                   value="1" 
                                   {{ old('is_active', $actor->is_active) ? 'checked' : '' }}>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    imageInput.addEventListener('change', function(event) {
        ...
        if (file && file.type.startsWith('image/')) {
            ...
            imagePreview.src = "{{ $actor->image_path ? Storage::url($actor->image_path) : asset('assets/images/actor-placeholder.png') }}";
            imageFileName.textContent = "{{ $actor->image_path ? 'Ảnh hiện tại: ' . basename($actor->image_path) : 'Chưa chọn file ảnh' }}";
            if (file) {
    ...
    if (removeImageCheckbox) {
        removeImageCheckbox.addEventListener('change', function() {
            if (this.checked) {
                ...
                imageFileName.textContent = 'Ảnh sẽ bị xóa';
                imageInput.value = '';
            } else {
                imagePreview.src = "{{ $actor->image_path ? Storage::url($actor->image_path) : asset('assets/images/actor-placeholder.png') }}";
                imageFileName.textContent = "{{ $actor->image_path ? 'Ảnh hiện tại: ' . basename($actor->image_path) : 'Chưa chọn file ảnh' }}";
