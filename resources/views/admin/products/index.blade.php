        ...
        <div class="col-12">
            <div class="d-flex align-items-center justify-content-between flex-wrap gap-3">
                ...
                <div class="d-flex gap-2 flex-wrap">
                    @can('create product')
                        <a href="{{ route('admin.products.create') }}" class="btn btn-primary btn-lg rounded-pill">
                ...
                <div class="card-body p-4">
                    <div class="d-flex align-items-center">
                        ...
                        <div class="flex-grow-1 ms-3">
                            <h6 class="mb-0 text-muted fw-semibold">Tổng sản phẩm</h6>
                            <h3 class="mb-0 fw-bold" style="color: var(--primary-teal);">{{ $products->total() }}</h3>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-6 col-md-4">
                            <div class="d-flex gap-2 justify-content-end flex-wrap">
                                <form class="d-flex align-items-center gap-2" method="GET" action="{{ route('admin.products.index') }}">
                        ...
                        <table class="table table-hover mb-0">
                            <thead class="table-light">
                                <tr>
                                    ...
                                    <th class="border-0 fw-bold text-dark">
                                        <i class="fas fa-box-open me-1" style="color: var(--accent-yellow);"></i> Tên sản phẩm
                                    </th>
                                    <th class="border-0 fw-bold text-dark">
                                        <i class="fas fa-barcode me-1" style="color: var(--primary-teal);"></i> SKU
                                    </th>
                                    <th class="border-0 fw-bold text-dark">
                                        <i class="fas fa-align-left me-1" style="color: var(--accent-yellow);"></i> Mô tả
                                    </th>
                                    <th class="border-0 fw-bold text-dark">
                                        <i class="fas fa-tag me-1" style="color: var(--primary-orange);"></i> Danh mục
                                    </th>
                                    <th class="border-0 fw-bold text-dark">
                            ...
                            <tbody>
                                @forelse ($products as $product)
                                    <tr class="border-bottom border-light">
                                        ...
                                        <td>
                                            <span class="fw-medium"><i class="fas fa-box-open me-2 text-orange"></i>{{ $product->name }}</span>
                                        </td>
                                        <td>{{ $product->sku ?? 'Chưa có SKU' }}</td>
                                        <td>{{ Str::limit($product->description, 50) }}</td>
                                        <td>{{ $product->category->name ?? 'Chưa có danh mục' }}</td>
                                        <td>
                                            <span class="text-muted"><i class="fas fa-calendar-alt me-2"></i>{{ $product->created_at->format('d/m/Y H:i') }}</span>
                                        </td>
                                        <td class="text-center pe-4">
                                            <div class="d-flex gap-1 justify-content-center">
                                                @can('edit product')
                                                    <a href="{{ route('admin.products.edit', $product->id) }}"
                                                       class="btn btn-sm edit-btn" data-bs-toggle="tooltip" title="Chỉnh sửa">
                                                        <i class="fas fa-edit"></i>
                                                    </a>
                                                @endcan
                                                <a href="{{ route('admin.products.show', $product->id) }}"
                                                   class="btn btn-sm view-detail-btn" data-bs-toggle="tooltip" title="Xem chi tiết">
                    ...
                    <div class="card-footer bg-white border-top py-4 rounded-bottom-4">
                        <div class="d-flex justify-content-between align-items-center">
                            <div class="text-muted small">
                                Hiển thị {{ $products->firstItem() }}-{{ $products->lastItem() }} trong tổng {{ $products->total() }} sản phẩm
                            </div>
                            <div>
