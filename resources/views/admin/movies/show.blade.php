                                    ...
                                    <div class="row g-4 align-items-end">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            @if($movie->end_date)
                                                <small class="text-muted mt-1 d-block">
                                                    ...
                                                    Giới hạn đến: {{ $movie->end_date->format('d/m/Y') }}
                                                </small>
                                            @endif
                                            </label>
                                            <input type="date" 
                                                   ...
                                                   @if($movie->end_date) max="{{ $movie->end_date->format('Y-m-d') }}" @endif
                                                   required>
                                            ...
                                            @if($movie->end_date && now()->format('Y-m-d') > $movie->end_date->format('Y-m-d'))
                                                <div class="alert alert-warning mt-2 p-2" style="font-size: 0.875rem;">
                                                    <i class="bx bx-info-circle me-1"></i>
                                                    <strong>Lưu ý:</strong> Phim đã hết hạn chiếu ({{ $movie->end_date->format('d/m/Y') }}). Không thể tạo suất chiếu mới.
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="col-md-2">
                                            <div class="d-flex gap-2">
                                                @if($movie->status->value === 'ended' || ($movie->end_date && now()->format('Y-m-d') > $movie->end_date->format('Y-m-d')))
                                                    <button type="button" 
                                                            class="btn btn-secondary btn-lg flex-fill rounded-3 shadow-sm" 
                                                            disabled
                                                            title="{{ $movie->status->value === 'ended' ? 'Không thể tạo suất chiếu cho phim đã kết thúc' : 'Không thể tạo suất chiếu sau ngày kết thúc phim' }}">
                                    ...
                                    <table class="table table-hover align-middle mb-0">
                                        <thead class="bg-light">
                                            <tr>
                                                ...
                                                <th class="border-0 fw-semibold text-dark py-3">Vé đã đặt</th>
                                                    ...
                                                    <td>
                                                        ...
                                                        @if($bookedTicketsCount > 0)
                                                            <span class="badge bg-info rounded-pill px-3 py-2">
                                                                <i class="bx bx-ticket me-1"></i>{{ $bookedTicketsCount }} vé
                                                            </span>
                                                        @else
                                                            <span class="text-muted">
                                                                <i class="bx bx-minus"></i> Chưa có vé
                                                            </span>
                                                        @endif
                                                    </td>
                                                    <td>
                                                        ...
                                                        <div class="d-flex gap-1">
                                                            ...
                                                            @else
                                                                <button type="button" 
                                                                        ...
                                                                        disabled
                                                                        title="@if($showtime->hasBookedTickets()) Không thể chỉnh sửa - Đã có vé được đặt @else Không thể chỉnh sửa - Suất chiếu {{ ucfirst($statusValue) }} @endif">
                                                                    <i class="bx bx-edit fs-16"></i>
                                                                </button>
                                                            @endif
                                                            
                                                            <!-- @if($statusValue === 'scheduled' && !$showtime->hasBookedTickets())
                                                                <button type="button" 
                                                            ...
                                                            @elseif($statusValue === 'scheduled' && $showtime->hasBookedTickets())
                                                                <button type="button" 
                                                                        class="btn btn-sm btn-outline-warning rounded-pill" 
                                                                        disabled
                                                                        title="Không thể hoãn - Đã có vé được đặt">
                                                                    <i class="bx bx-time-five fs-16"></i>
                                                                </button>
                                                                ...
                                                                <button type="button" 
                                                                        class="btn btn-sm btn-outline-danger rounded-pill" 
                                                                        disabled
                                                                        title="Không thể hủy - Đã có vé được đặt">
                                                                    <i class="bx bx-x fs-16"></i>
                                                                </button>
                                                            @endif
                                                            
                                                            @if($statusValue === 'postponed' && !$showtime->hasBookedTickets())
                                                                <button type="button" 
                                                            ...
                                                            @elseif($statusValue === 'postponed' && $showtime->hasBookedTickets())
                                                                <button type="button" 
                                                                        class="btn btn-sm btn-outline-success rounded-pill" 
                                                                        disabled
                                                                        title="Không thể kích hoạt lại - Đã có vé được đặt">
                                                                    <i class="bx bx-check fs-16"></i>
                                                                </button>
                                                            @endif -->

                                                            @if($showtime->hasBookedTickets())
                                                                <span class="badge bg-info rounded-pill ms-2" title="Suất chiếu đã có vé được đặt">
                                                                    <i class="bx bx-check-circle fs-12"></i> Đã có vé
                                                                </span>
                                                            @endif
                                                        </div>
                                                    </td>
...
// Validation cho input ngày khi người dùng thay đổi
document.getElementById('date').addEventListener('change', function() {
    const selectedDate = this.value;
    @if($movie->end_date)
    const movieEndDate = '{{ $movie->end_date->format("Y-m-d") }}';
    
    if (selectedDate > movieEndDate) {
        alert(`Không thể chọn ngày sau ngày kết thúc phim (${new Date(movieEndDate).toLocaleDateString('vi-VN')}). Vui lòng chọn ngày khác.`);
        this.value = '{{ now()->format("Y-m-d") }}'; // Reset về ngày hiện tại
        return false;
    }
    
    // Cảnh báo nếu chọn ngày gần ngày kết thúc (trong vòng 3 ngày)
    const selectedDateObj = new Date(selectedDate);
    const endDateObj = new Date(movieEndDate);
    const daysDiff = Math.ceil((endDateObj - selectedDateObj) / (1000 * 60 * 60 * 24));
    
    if (daysDiff <= 3 && daysDiff >= 0) {
        const confirmMsg = `Bạn đang chọn ngày gần ngày kết thúc phim (còn ${daysDiff} ngày). Bạn có chắc chắn muốn tiếp tục?`;
    ...
    if (selectedDate < today) {
        alert('Không thể chọn ngày trong quá khứ. Vui lòng chọn ngày khác.');
        this.value = today; // Reset về ngày hiện tại
        return false;
    }
});
...
document.querySelector('[data-bs-target="#confirmShowtimeModal"]').addEventListener('click', function(e) {
    ...
    // Kiểm tra ngày có sau ngày kết thúc phim không
    @if($movie->end_date)
    const movieEndDate = '{{ $movie->end_date->format("Y-m-d") }}';
    if (dateInput > movieEndDate) {
        const errorModal = new bootstrap.Modal(document.getElementById('errorModal'), {});
        document.getElementById('errorMessage').textContent = `Không thể tạo suất chiếu sau ngày kết thúc phim (${new Date(movieEndDate).toLocaleDateString('vi-VN')}).`;
        errorModal.show();
        return;
    }
    @endif
...
document.getElementById('confirmCreateShowtime').addEventListener('click', function() {
    ...
    // Kiểm tra ngày kết thúc phim trước khi submit
    @if($movie->end_date)
        const today = '{{ now()->format("Y-m-d") }}';
        const movieEndDate = '{{ $movie->end_date->format("Y-m-d") }}';
        
        if (today > movieEndDate) {
            alert('Không thể tạo suất chiếu vì phim đã hết hạn chiếu!');
            return false;
        }
    @endif
