...
<section class="w3l-grids">
    <div class="grids-main py-5">
        <div class="container py-lg-3">
            <div class="headerhny-title">
                <div class="w3l-title-grids d-flex justify-content-between align-items-end flex-wrap gap-3">
                    <div class="headerhny-left d-flex align-items-center gap-2">
                        <h3 class="hny-title mb-0">Phim Đang Chiếu</h3>
                        <small class="text-muted">({{ $showingMovies->count() }} phim)</small>
                    </div>
                    <div class="d-flex gap-2 align-items-end flex-wrap">
                        <div>
                            <label for="filter-date" class="form-label fw-bold mb-1">Ngày:</label>
                            <input type="date" id="filter-date" class="form-control form-control-sm" style="width: 140px;" value="{{ request('date') }}">
                        </div>
                        <div>
                            <label for="from-time" class="form-label fw-bold mb-1">Từ:</label>
                            <input type="time" id="from-time" name="from_time" class="form-control form-control-sm" style="width: 120px;" value="{{ request('from_time') }}">
                        </div>
                        <div>
                            <label for="to-time" class="form-label fw-bold mb-1">Đến:</label>
                            <input type="time" id="to-time" name="to_time" class="form-control form-control-sm" style="width: 120px;" value="{{ request('to_time') }}">
                        </div>
                        <div class="align-self-end">
                            <button class="btn btn-sm btn-danger" onclick="applyFilters()">Lọc phim</button>
                        </div>
                        <div class="align-self-end">
                            <h4 class="mb-0">
                                <a class="show-title" href="{{ route('movies.show', ['id' => $showingMovies->first()->id ?? 1]) }}">Xem tất cả</a>
                            </h4>
                        </div>
                    </div>
                </div>
            ...
            <div class="w3l-populohny-grids mt-4" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                @if($showingMovies->isEmpty())
                    <div class="col-12 text-center">
                        <div class="alert alert-warning w-100 mt-3" role="alert">
                            😥 Không tìm thấy phim nào phù hợp với bộ lọc bạn đã chọn.
                        </div>
                    </div>
                @else
    ...
    <script>
        function applyFilters() {
            ...
            if (fromTime) url.searchParams.set('from_time', fromTime); else url.searchParams.delete('from_time');
            if (toTime) url.searchParams.set('to_time', toTime); else url.searchParams.delete('to_time');
    ...
    <div class="grids-main py-5">
        <div class="container py-lg-3">
            ...
            <div class="w3l-populohny-grids" style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px;">
                ...
                @else
                @foreach($showingMovies->unique('id') as $movie)
                <!-- giữ nguyên phần hiển thị từng phim -->
                @endforeach
                @endif
