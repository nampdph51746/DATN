    ...
    <div class="col-lg-4">
      <div class="card border-0 shadow-sm rounded p-3">
        ...
        <div class="d-flex gap-2">
          @if ($cinema->trashed())
          <a href="{{ route('cinemas.trash') }}" class="btn btn-soft-secondary btn-sm" title="Quay lại danh sách rạp đã xóa">
            <iconify-icon icon="solar:arrow-left-broken" class="fs-18"></iconify-icon>
          </a>
          @else
          <a href="{{ route('cinemas.index') }}" class="btn btn-soft-secondary btn-sm" title="Quay lại danh sách rạp">
            <iconify-icon icon="solar:arrow-left-broken" class="fs-18"></iconify-icon>
          </a>
          @endif

          @if(!$cinema->trashed())
          <a href="{{ route('cinemas.edit', $cinema->id) }}" class="btn btn-soft-primary btn-sm" title="Chỉnh sửa">
            <iconify-icon icon="solar:pen-2-broken" class="fs-18"></iconify-icon>
          </a>

          <form action="{{ route('cinemas.destroy', $cinema->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa rạp này?')">
            @csrf
            @method('DELETE')
          ...
          @else
          <form action="{{ route('cinemas.forceDelete', $cinema->id) }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xóa vĩnh viễn rạp này?')">
            @csrf
            @method('DELETE')
