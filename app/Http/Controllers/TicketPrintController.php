...
class TicketPrintController extends Controller
{
    ...
    {
        $ticket = Ticket::with(['showtime.movie', 'showtime.cinema', 'showtime.room', 'seat.seatType', 'booking'])
    ...
    {
        $booking = Booking::with(['bookingItems.productVariant.product'])
            ->where('booking_code', $booking_code)
            ->firstOrFail();

        $bookingItems = $booking->bookingItems;

        // Sinh QR code chi tiết cho booking items
        $qrCodeBase64 = null;
        if ($booking && $bookingItems->count() > 0) {
            $qrService = app(QrcodeService::class);
            $qrText = $qrService->generateFoodQrCode($bookingItems);
            $qrCodeRaw = $qrService->generateQrCode($qrText, 120);
            if ($qrCodeRaw) {
                $qrCodeBase64 = 'data:image/png;base64,' . $qrCodeRaw;
            }
        ...
        $pdf = Pdf::loadView('admin.booking_items.print', [
            'booking' => $booking,
            'bookingItems' => $bookingItems,
            'qrCodeBase64' => $qrCodeBase64
        ]);

        return $pdf->download('do-an-' . $booking_code . '.pdf');
    }
