    ...
    <div class="col-md-6 col-xl-3">
        <div class="card border shadow-sm">
            <div class="card-body d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1">Tất cả</h5>
                    <p class="fs-5 mb-0">{{ $ticketStats['all'] }}</p>
                </div>
                <div class="avatar-md bg-secondary bg-opacity-10 rounded">
                    <iconify-icon icon="mdi:clipboard-list-outline" class="fs-32 text-secondary avatar-title"></iconify-icon>
                </div>
            </div>
        </div>
    </div>

    {{-- Các trạng thái từ enum --}}
    @php
        $statusLabels = [
            'valid' => ['label' => 'Hợp lệ', 'color' => 'success', 'icon' => 'mdi:ticket-confirmation-outline'],
            'used' => ['label' => 'Đã sử dụng', 'color' => 'primary', 'icon' => 'mdi:check-circle-outline'],
            'cancelled' => ['label' => 'Đã hủy', 'color' => 'danger', 'icon' => 'mdi:cancel'],
        ];
    @endphp
            ...
            <div class="card border shadow-sm">
                <div class="card-body d-flex justify-content-between align-items-center">
                    <div>
                        ...
                        <p class="fs-5 mb-0">{{ $ticketStats[$key] ?? 0 }}</p>
                    </div>
                    <div class="avatar-md bg-{{ $info['color'] }} bg-opacity-10 rounded">
                        <iconify-icon icon="{{ $info['icon'] }}" class="fs-32 text-{{ $info['color'] }} avatar-title"></iconify-icon>
                    </div>
                </div>
            </div>
        </div>
