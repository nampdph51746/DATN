        ...
        <div class="col-xl-12">
            <div class="card">
                <div class="card-header">
                    <div class="row align-items-center g-3">
                        <div class="col-md-4">
                            <h4 class="card-title mb-0">
                                <i class="bi bi-globe-americas me-2 text-info"></i>
                                🌎 Danh Sách Quốc Gia
                            </h4>
                        </div>
                        <div class="col-md-8">
                            <div class="d-flex flex-wrap gap-2 justify-content-md-end">
                                ...
                                <!-- Nút thêm -->
                                <a href="{{ route('admin.countries.create') }}" class="btn btn-sm btn-success">
                                    <i class="bi bi-plus-circle me-1"></i> Thêm quốc gia
                                </a>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Form xóa hàng loạt -->
                    <form id="delete-selected-form" action="{{ route('admin.countries.bulkDelete') }}" method="POST" style="display: none;" class="mt-2">
                        @csrf
                        @method('DELETE')
                        <input type="hidden" name="ids" id="selected-country-ids">
                        <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Bạn có chắc muốn xóa các quốc gia đã chọn?')">
                        ...
                        <table class="table align-middle mb-0 table-hover table-centered">
                            <thead class="bg-light-subtle">
                                <tr>
                                    ...
                                    <th style="width: 250px;">Thông tin quốc gia</th>
                            ...
                            <tbody>
                                @forelse($countries as $key => $country)
                                    <tr>
                                            ...
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <div>
                                                    <h6 class="mb-1 fw-semibold text-info">
                                                        <i class="bi bi-geo-alt me-2"></i>{{ $country->name }}
                                                    </h6>
                                        ...
                                        <td>
                                            <div>
                                                <div class="fw-medium">{{ $country->created_at->format('d/m/Y') }}</div>
                                                <small class="text-muted">{{ $country->created_at->format('H:i') }}</small>
                                            </div>
                                        </td>
                                        <td>
                                            <div>
                                                <div class="fw-medium">{{ $country->updated_at->format('d/m/Y') }}</div>
                                                <small class="text-muted">{{ $country->updated_at->format('H:i') }}</small>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex gap-2 justify-content-center">
                                                <a href="{{ route('admin.countries.edit', $country->id) }}" class="btn btn-soft-primary btn-sm" title="Chỉnh sửa quốc gia">
                                                    <iconify-icon icon="solar:pen-2-broken" class="align-middle fs-18"></iconify-icon>
                                                </a>
                                                <form action="{{ route('admin.countries.destroy', $country->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Bạn có chắc muốn xóa quốc gia này?')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-soft-danger btn-sm" title="Xóa quốc gia">
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="text-center text-muted py-4">
                                            ...
                                            <div>Không tìm thấy quốc gia nào.</div>
                                            <div class="mt-2">
                                                <a href="{{ route('admin.countries.create') }}" class="btn btn-sm btn-primary">
                                                    <i class="bi bi-plus-circle me-1"></i>Thêm quốc gia đầu tiên
                                                </a>
                                            </div>
                                        </td>
                                    </tr>
                ...
                <div class="card-footer border-top">
                    <div class="d-flex justify-content-end">
                        {{ $countries->appends(['keyword' => request('keyword')])->links('pagination::bootstrap-5') }}
                    </div>
                </div>
            </div>
        </div>
